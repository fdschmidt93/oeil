...
class Model(LightningModule):
    def __init__(self, model_name_or_path: str, lr=2e-5):
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(self.hparams.model_name_or_path, num_labels=2)
...
class Collator:
    ...
    def __call__(self, inputs):
        ...
        batch = self.tokenizer(documents, return_tensors='pt', padding=True, truncation=True)
...
train_loader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size, num_workers=4, collate_fn=collate_fn)
val_loader = DataLoader(val_dataset, shuffle=False, batch_size=batch_size, num_workers=4, collate_fn=collate_fn)
trainer = Trainer(gpus='0', max_epochs=15, callbacks=[ModelCheckpoint(monitor='val_acc', mode='max')])
...
other_dataset = PlainDataset(data=other_sum, labels=torch.ones((len(other_sum),)).long())
other_loader = DataLoader(other_dataset, shuffle=False, batch_size=batch_size, num_workers=4, collate_fn=collate_fn)
...
legis_sum_dataset = PlainDataset(data=all_sum, labels=torch.ones((len(all_sum),)).long())
legis_sum_loader = DataLoader(legis_sum_dataset, shuffle=False, batch_size=batch_size, num_workers=4, collate_fn=collate_fn)
...
final_act_dataset = PlainDataset(data=final_act_sum, labels=torch.ones((len(final_act_sum),)).long())
final_act_loader = DataLoader(final_act_dataset, shuffle=False, batch_size=batch_size, num_workers=4, collate_fn=collate_fn)
...
filtered_legis_sum_probs = np.around(torch.vstack(filtered_legis_sum_probs)[:, 1].numpy(), 2)
filtered_final_act_probs = np.around(torch.vstack(filtered_final_act_probs)[:, 1].numpy(), 2)
...
df_dico = {'COD': overlapping_paths,
            ...
            'Transformer_Prob_dif': np.around(np.array(filtered_final_act_probs) - np.array(filtered_legis_sum_probs), 2),
}
