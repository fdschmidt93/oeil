...
def main(config: DictConfig):
    ...
    lp_paths = get_summary_paths(config.data.summaries, final_act=False)
    ...
    #   train with `learn_tokenizers.py`
    ...
    # doc2  [   0  ,  3   , ...,   1    ]
    # doc3  [   2  ,  5   , ...,   1    ]
    lp_token_counts = np.stack([tokenize_to_vec(doc, tokenizer) for doc in lp_all_sum])

    lp_cod = [get_cod(p) for p in lp_train_paths + lp_other_paths]
    lp_token_counts_df = pd.DataFrame(
        index=lp_cod, data=lp_token_counts, columns=tokens
    )
    ...
    lp_tfidf = pd.DataFrame(index=lp_cod, data=X_lp_all, columns=tokens)
    ...
    lp_cod = [get_cod(p) for p in lp_other_paths]
    out = {
        "COD": lp_cod,
    ...
    fa_paths = get_summary_paths(config.data.summaries, final_act=True)
    fa_sum = [read_file(x) for x in fa_paths]

    fa_token_counts = np.stack([tokenize_to_vec(doc, tokenizer) for doc in fa_sum])
    lp_cod = [get_cod(p) for p in fa_paths]
    fa_token_counts_df = pd.DataFrame(
        index=lp_cod, data=fa_token_counts, columns=tokens
    )
    ...
    fa_tfidf = pd.DataFrame(index=lp_cod, data=X_fa, columns=tokens)
    ...
    lp_cod = [get_cod(p) for p in lp_paths]
    fa_cod = [get_cod(p) for p in fa_paths]
    #   Get the overlapping paths
    overlapping_paths = list(set(lp_cod).intersection(set(fa_cod)))
